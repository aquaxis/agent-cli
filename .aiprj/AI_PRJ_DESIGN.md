...
1. 一時ディレクトリで隔離された`registry_dir`／`log_dir`を用意。
2. `--provider`で指定されたバックエンド（未指定時は設定の`provider.kind`）でProviderを構築。
3. 短いプロンプト（例：「Reply with the literal text OK.」）を投げ、応答に`OK`が含まれることを確認。
4. シェルツール実行を要求するプロンプト（例：`echo selftest`を実行）を投げ、tool_use → ツール実行 → 結果反映までの一巡が完了することを確認。
5. 別プロセスを内部起動し、IPC経由のプロンプト授受が成功することを確認。
6. すべて成功で終了コード0、失敗で非0を返す。
