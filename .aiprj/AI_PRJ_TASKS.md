...
### T-507 REPL UX 仕上げ：`/exit` エイリアスと改行・プロンプト再描画（FR-13／FR-03-2／設計書 4.2A）[ ]

別ホスト実機検証（FR-09-2）で受領した 2 件の要望に対応する。

- `/exit でも終了したい`：現状コードの `match cmd { "quit" | "exit" => return false, ... }` で動作するが、`/help` 出力やドキュメントに `/exit` が明示されておらず発見性が低い。
- `応答があるとプロンプトマーク "> " の後に表示される / 応答後に改行してプロンプト再描画してほしい`：T-505 でプロンプト同期は実装済だが、視覚レイアウトの保証（FR-03-2 設計書 4.2A の不変条件）が成立することを単体テストと実機で再確認する必要がある。報告は別ホスト側の旧バイナリで観測された可能性が高いため、新バイナリでの検証＋テストで覆う。

- [ ] `/help` 出力に `/exit` を `/quit` のエイリアスとして併記
- [ ] `/help` 出力に承認スキップ手段（`/auto`／`auto_approve_tools`／`--auto-approve-tools`）も併記（T-506 と整合）
- [ ] `doc/usage.md` の REPL コマンド章に `/exit` を併記（T-602-2 と整合）
- [ ] 単体テスト：`exit_command_terminates_input_loop` — `/exit` 投入で `run_input_loop` が break し `shutdown_tx.send(true)` が発行される（既存 `/quit` テストの並行版）
- [ ] 単体テスト：`prompt_not_drawn_between_user_input_and_response` — `Pending` 中に `print_prompt` が呼ばれないことを観測（描画呼び出しを `Arc<AtomicUsize>` カウンタで追跡し、user input 投入から idle 通知までの間カウンタが増えないことを assert）
- [ ] 単体テスト：`done_emits_trailing_newline_then_prompt` — `display_event(AgentEvent::Done)` が改行を出し、その後の `Ready` 遷移で `> ` が描画される並びを assert
- [ ] 手動：別ホストの最新バイナリで対話 → ユーザー入力直後に `> ` が前置されないこと、応答後に改行 → `> ` が表示されることを目視確認
- 検証は T-704（別ホストワンライナー導入検証）と連動

### T-506 ツール承認の入出力統合と `/auto` REPL コマンド（FR-04-1／FR-04-2／設計書 4.3A）[ ]

別ホストでの実機検証（FR-09-2）において、(1)「ツール実行の許可を解除する設定方法はあるか」というユーザー質問、(2)「ツール実行後に応答が無い」現象（実態は承認 `y` 入力が次のプロンプトとして取り違えられて承認失敗→ツール拒否→AI が代替案で謝罪する流れ）が報告された。`agent.rs::approval_prompt` が `std::io::stdin().read_line()` を直接呼んでおり、`run_input_loop` の `tokio::io::stdin()` と stdin を奪い合うことが根本原因。

- [ ] `ApprovalRequest { name, args, response: oneshot::Sender<bool> }` を新規定義
- [ ] `Agent` 構造体に `approval_tx: Option<mpsc::Sender<ApprovalRequest>>` を追加
- [ ] `Agent::process_turn` の承認分岐を `approval_tx` 経由の oneshot 待ち合わせに変更し、`std::io::stdin` 直読みを排除
- [ ] `auto_approve` を `Arc<AtomicBool>` 化し、agent と REPL コマンドで共有
- [ ] `enum PromptState` を `Ready / Pending / AwaitingApproval(oneshot::Sender<bool>)` の 3 値に拡張
- [ ] `run_input_loop` に `approval_rx` 引数を追加し、`tokio::select!` の arm を 4 本（shutdown／idle／approval／stdin）に拡張
- [ ] `AwaitingApproval` 状態で stdin 行を `y/Y/yes/Yes/YES` 判定し oneshot で返答、状態を `Pending` に遷移
- [ ] shutdown 経路で `AwaitingApproval` を抜ける際は `resp_tx.send(false)` で安全側倒し
- [ ] REPL コマンド `/auto on|off|status` を実装（引数なしは現在値表示）
- [ ] `/help` の出力に `auto_approve_tools` の 3 経路（設定ファイル／CLI フラグ／`/auto`）を明記
- 検証：
  - [ ] 単体テスト：`approval_y_resolves_true` — `ApprovalRequest` を入力ループへ送り、stdin "y\n" 投入で oneshot が true で完了
  - [ ] 単体テスト：`approval_other_resolves_false` — 同上で "no\n" 投入で false
  - [ ] 単体テスト：`approval_blocks_user_prompt` — AwaitingApproval 中の入力が `UserPrompt` として agent へ流出しない
  - [ ] 単体テスト：`auto_atomic_skips_approval_path` — `auto_approve.store(true)` で承認チャネルを経由せずに即承認される
  - [ ] 既存 47 テスト＋4 経路終了テストに回帰なし
  - [ ] 手動：`auto_approve = false` で天気照会のような shell tool 要求 → `y` で承認 → ツール実行 → AI 応答が確実に成立する
  - [ ] 手動：REPL 起動中に `/auto on` → 続くツール要求が承認なしで自動実行される
...
- 検証：
  ...
  - [~] 別ホストで `curl ... | sh`（実機検証進行中）
    ...
    - 「ツール実行の許可を解除する設定方法はあるか」という質問を受領 → T-506 で対応予定（`/auto` コマンド追加と `/help` 改善）
    - 「ツール実行してから応答が無い」事象を発見（承認 `y` が次のプロンプトとして取り違えられる）→ T-506 で対応予定
    - 「`/exit` でも終了したい」要望を受領 → T-507 で対応（コード上は既に `quit | exit` で動作、`/help` とドキュメントへ併記）
    - 「応答後に改行してプロンプト再描画してほしい」要望を受領 → T-507 で視覚レイアウト保証を再検証（T-505 で実装済の振る舞いの確認）
    - 開発機上での 4 経路（`/quit`／`Ctrl+D`／`SIGINT`／`SIGTERM`）回帰テストはすべて 1 秒以内に正常終了し、レジストリ残留物なしを確認
    - 残作業：T-506／T-507 修正後の再検証、`agent-cli --help`／`agent-cli doctor`／実プロバイダ対話の確認（T-704）
...
- [ ] 別ホストでユーザー入力直後に `> ` が前置されず、応答終了後に改行＋`> ` が描画されることを目視確認（T-507／FR-03-2）
- [ ] 別ホストで `/exit` で終了することを確認（T-507／FR-13）
- [ ] 別ホストでシェルツール承認 `y` 入力が確実に承認として処理されることを確認（T-506 修正後／FR-04-1）
