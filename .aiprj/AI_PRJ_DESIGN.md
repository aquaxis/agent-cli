...
- 取得方法：
  ...
  - そうでなければ環境変数`AGENT_CLI_REPO`（既定`https://github.com/<owner>/agent-cli.git`）と`AGENT_CLI_REF`（既定`main`）から一時ディレクトリへ`git clone`し、`cargo install --path . --root $AGENT_CLI_PREFIX`を実行
- インストール先：
...
| 変数 | 既定値 | 説明 |
|------|--------|------|
| `AGENT_CLI_REPO` | `https://github.com/<owner>/agent-cli.git` | 取得元リポジトリ |
