...
├── theme.rs             ... colour palette: Role -> ANSI 16 colour + attributes / paint / ColorMode / Theme
├── scroll.rs            ... session transcript (ring buffer) / SGR-aware wrap / scroll viewport arithmetic
├── select.rs            ... mouse selection of the log: range arithmetic / highlight / text extraction
├── clip.rs              ... clipboard delivery: OSC 52 (base64, tmux passthrough) or `[ui] copy_command`
├── permissions.rs       ... `[permissions]` rules: parse / match a call's gated argument / Decision
├── swarm.rs             ... how far an agent may go in creating agents: SpawnLimits / spawn_allowance
├── shell.rs             ... `!<command>`: line classification / ShellRun (spawn, stream, timeout, cancel) / context message
...
│   ├── spawn.rs         ... detached peer creation (opt-in, not enabled by default)
│   ├── list_agents.rs   ... the agents this one created (own subtree only)
│   ├── stop_agent.rs    ... stop an agent this one created (own subtree only)
...
- **Turn progress indicator**: `process_turn` opens every turn with `AgentEvent::TurnStart`, which prints nothing and exists solely to mark the boundary. `display_task` is a `tokio::select!` over the event channel and a 100 ms `interval`; the tick advances a `StatusIndicator` (in `app.rs`) that owns the bottom row of the output — a spinner plus the elapsed time since `TurnStart`, redrawn in place under the line describing what is running (`MoveToColumn(0)` + `Clear(FromCursorDown)`, so the erase is exact). The row is cut to `terminal_width - 1` columns so it can never wrap, and while the indicator is enabled (`DisplayState::compact_output`) the neighbouring output is kept short too: `[tool-call]` is cut to one row and `[tool-result]` to `TOOL_RESULT_ROWS` (5) wrapped rows plus a `… +N more lines` marker (`compact_tool_result`), screen-only — the model and the conversation log still get the whole result. It is painted only from column 0 of a fresh row — `event_ends_at_line_start` decides that per event, so a half-written row of streamed text is never disturbed — and it is cleared before any other output is written. On the terminal event it collapses to one permanent `✔ <elapsed>` (`✗` after an error); a cancelled turn's suppressed events call `abandon_run`, leaving no mark. The indicator is shared with the input loop as an `Arc<Mutex<..>>` so the loop can `suspend()` it (clearing the row) before printing `[cancelled]`, the approval prompt, or on shutdown, and `resume()` it when the approval answer returns the REPL to `Pending`; the lock is only ever held across synchronous writes. It is enabled only when stdin and stderr are both TTYs and `[ui] show_progress` is true, and is a complete no-op otherwise (`serve` mode never constructs one).
- **Live `thinking` view**: while the indicator is on (and `[ui] show_thinking` is not `hidden`), `AgentEvent::Thinking` is no longer printed inline — `display_event` skips it and the display task feeds it to the indicator, which buffers the turn's reasoning and renders its tail beneath the spinner. `editor::wrap_display` folds the text to `terminal_width - 3` columns, the last `THINKING_ROWS` (10) rows are shown with a `… +N more` marker, and the whole block is bounded by `terminal_height - 5` so expanding can never push the conversation off the screen. Because the block is now several rows tall, `clear` moves up `painted_rows - 1` before erasing. Clicking it toggles the tail view against the full one. Who owns mouse reporting depends on `[ui] mouse_scroll`: with the wheel scrollback live (the default) the input loop enables reporting for the whole raw-mode session and the indicator's own switch is disabled (`owns_mouse`, see below), so text selection needs Shift throughout; with `mouse_scroll = false` the indicator enables reporting in `start()` and releases it in `finish()` / `abandon_run()`, so Shift is only needed while a turn is actually running. `RawModeGuard`'s `Drop` disables it either way, as a safety net. The input loop reads `CtEvent::Mouse`, resolves the block's rows with one `cursor::position()` query — the display task is the last writer during a turn, so the cursor stands on the block's final row — and tests the click with the pure `click_hits_indicator`.
- **Colour scheme**: `theme.rs` holds the whole palette as a mapping from a `Role` (what a piece of text *is* — `PromptSymbol`, `ToolName`, `ToolArgs`, `Progress`, `Success`, `Confirm`, …) to an ANSI 16 foreground colour and its attributes, plus the pure `paint(role, text)` that wraps a string in the SGR sequence and closes it with a reset. `ColorMode { Auto, Always, Never }` comes from `[ui] color`, and the pure `resolve(mode, is_tty, NO_COLOR, TERM)` is evaluated **once per stream at startup** into a `Copy` `Theme { stdout, stderr }`, which is carried on `ReplState`, `DisplayState` and `StatusIndicator`. `Theme::out` / `Theme::err` return `Cow::Borrowed` unchanged when that stream has no colour, so a plain session is byte-identical to an uncoloured build and `run_headless` simply uses `Theme::plain()`. Two invariants make it safe next to the display maths: **paint last** — `str_display_width`, `truncate_display` and `wrap_display` (in `editor.rs`) count every character, so styling is applied only after a line has been measured, cut and wrapped, and `PromptRenderer` keeps computing its cursor column from the *plain* prompt and buffer — and **one sequence per row**: multi-row output (`style_rows`, the indicator's block) is styled row by row, so a colour never spans a line break that may later be erased and redrawn. The `[tool-call]` line is the one composite: `style_tool_call_line` splits the finished line on its second space, colouring `[tool-call] <name>` and the arguments separately. Answer text (`AgentEvent::Text`) is deliberately never styled.
- **Reported results (`IpcMessage::Context`)**: a peer message can be delivered as a *report* instead of a prompt. `Prompt` makes the receiver run `process_turn` — with no `reply_to` that is a provider call whose answer has no recipient, so N children reporting in cost the parent N wasted turns. `Context { from, from_name, text }` (the same field names `Prompt` uses) is mapped by the IPC readers in **both** loops — `run` and `run_headless`, since a child may have children of its own — onto the `AgentInput::Context` the `!` shell path already uses: the agent appends one `Message::User`, writes `LogEvent::PeerContext`, and does nothing else. The text is built by the pure `agent::peer_context_text`, which gives it the same `[peer report from <name> (<id>)]` header shape a peer prompt has, so a model receiving several can tell them apart. The only event emitted is one `AgentEvent::Info` naming the sender and the size — a result that produced nothing on screen would be indistinguishable from a lost one, while printing several thousand-character reports would bury the session. No `TurnStart`, no `Done`, no idle notification, so the progress indicator never starts and the REPL is never released from anything. `send_to` chooses with `delivery` (`"prompt"` | `"report"` | `"ask"`, resolved ahead of the deprecated `wait_reply` alias). Adding a variant leaves the other seven messages' encoding untouched; an agent-cli that predates it answers `Error { "parse error: unknown variant ..." }` and stays up, which the tool uses as the signal to resend the text as a prompt and say so, rather than failing the sender's turn.
- **Agent trees (`spawn` / `list_agents` / `stop_agent`)**: a peer's `RegistryEntry` carries `ancestors` — the chain of agents that created it, root first — so `parent()` is its last element and `depth()` its length. The whole chain is kept rather than just the parent because an agent in the middle can exit: `list_entries` prunes its file, and its children would otherwise become unattributable. `descendants_of` is therefore a filter (`is_descendant_of` / `distance_from`) rather than a walk, and orphaned grandchildren stay in the tree that created them. The chain reaches a new peer through the environment — `spawn_detached` sets `AGENT_CLI_ANCESTORS` on the child's command, never on its argument vector, so every existing subcommand's behaviour is untouched — and the child reads it once at startup (`ancestors_from_env`) and writes it into the entry it registers. The field is `#[serde(default, skip_serializing_if = "Vec::is_empty")]`, so registry files written before it existed load as roots and an agent a human started serialises exactly as it always did. `swarm.rs` holds the policy as one pure function, `spawn_allowance(live_children, depth, limits)`: `[spawn] max_children` (live direct children, counted from the registry at call time, so an exited child has already freed its slot) and `max_depth`, with depth answered first because no amount of stopping children changes it. It is called by the **`spawn` tool** before anything is created — `agent-cli spawn` and `/spawn` are deliberately unbounded — and a refusal is a `ToolOutput::err` naming the limit, its value and the count. `stop_agent` resolves the target like `agent-cli stop` does, then requires it to be in the caller's subtree (`stop_permission`, pure) before delegating to `commands::stop_peer`, so there is one implementation of stopping and an agent can only undo what it did.
- **`!` shell commands**: a submitted prompt line is classified by the pure `shell::classify_line(line, enabled)` into `Command` (`/…`), `Shell` (`!…`), `ShellUsage`, `Prompt` or `Empty`. That call happens **only inside the two input loops**, on a line the user typed; a peer's prompt is wrapped as `AgentInput::PeerPrompt` by the IPC reader and never passes through it, so no remote agent — and nothing the model produced — can reach command execution. There is no approval gate either: the gate exists because a tool call is the *model's* decision, and a typed `!` is the user's (the model's own shell access is still the `bash` tool). `shell::ShellRun` spawns `bash -lc <command>` with stdout and stderr piped and **stdin closed**, in its own process group, and forwards each line on a channel; the raw loop drives it from a `select!` arm, so output streams through the ordinary `raw_*` writers (wrapped, coloured, in the scrollback transcript) while keys keep being polled — which is what lets `Esc` cancel and `[shell] timeout_ms` fire. Cancelling signals the whole process group, since `bash -lc` forks for the command it was given; the same happens on shutdown, so a command is stopped rather than orphaned. The terminal is never handed over: raw mode stays on and `Ctrl-C` stays a key rather than a signal that would trigger the REPL's own shutdown, which is also why interactive and full-screen programs are out of scope. When the command ends, `shell::context_message` renders the command, its status and the output (capped by `[shell] max_output_kb`, keeping the tail) and the loop sends it as **`AgentInput::Context`** — the one input that appends a `Message::User` to the conversation and does nothing else: no provider request, no `AgentEvent`, no turn. The conversation log records it as `LogEvent::Shell`.
- **Mouse-wheel scrollback**: `scroll.rs` keeps a `Transcript` — a `VecDeque` of the lines the REPL has printed, bounded by `[ui] scrollback_lines` and holding them exactly as they went to the terminal, styling included. It is fed at the one seam every REPL message already passes through: the four `raw_*` writers in `app.rs` tee into it before the CR+LF rewriting (`record` leaves a line open when a chunk has no trailing newline, so streamed answer fragments form one line), joined by `PromptRenderer::finish_line` (the echo of a submitted line) and `StatusIndicator::finish` (the `✔ <elapsed>` row). What is erased is deliberately **not** recorded: the indicator's repainted block and the prompt block being edited. The buffer is a process-wide handle installed only by the interactive REPL — the writers are free functions called from several tasks — so the piped loop and `run_headless` keep nothing and pay nothing. Everything else in the module is pure and unit-tested without a terminal: `wrap_styled` (SGR-aware wrapping — a sequence costs no column, is never split, and every produced row re-opens the style and closes with a reset), `marked_rows` / `max_offset` / `body_rows` (the viewport arithmetic, wrapping from the end so a screenful costs a screenful however long the session, each row carrying whether it continues the line above it), and `ScrollView` (the offset, its clamping, and whether a wheel notch changed anything). In `app.rs`, `ScrollOverlay` turns that into a screen: the first wheel-up suspends the indicator, clears the prompt block and switches to the **alternate screen**, each draw paints the transcript slice row by row with the prompt block pinned beneath it (through `PromptRenderer`, so the cursor arithmetic has one implementation) or, while a turn runs, the indicator's spinner row, and returning to offset 0 — or `Esc`, `Enter`, or leaving the REPL — switches back, which restores the main screen and the terminal's own scrollback untouched. Mouse reporting has to be on at the prompt as well, so when the feature is live the input loop owns it for the whole raw-mode session and `StatusIndicator::set_mouse_capture` is disabled (`owns_mouse`); `RawModeGuard::drop` still turns it off exactly once. Keys keep their meaning: only `Esc` is consumed by the scrolled view (so the first press returns to the live one and a second cancels), and `↑` / `↓` remain history navigation.

- **Permission rules**: `permissions.rs` is the second gate on a tool call, and the finer one — the first (`ToolRegistry::build`, `[tools] enabled` ∩ persona lists) decides which tools exist at all, once, at startup. It is pure: `Rule::parse` turns `tool` / `tool(pattern)` into a canonical tool name plus one of four `Pattern`s (`Any`, a command-word `Prefix`, a URL `Domain`, or a `Glob` over `globset`), `subject_of` names the single argument each tool is gated on (`command` for bash/monitor, `file_path` for read/write/edit, `path` — never the *search* `pattern` — for glob/grep, the host of `url` for webfetch, `peer` for send_to/stop_agent, and `None` for the rest), and `Ruleset::decide` scans deny, then allow, then `default_mode`. Path subjects are matched both as written and as a lexically resolved absolute path, so `read(.env*)` is not defeated by `./.env`; resolution is lexical rather than `canonicalize` because `write` and `edit` create files. The one impure edge is in `agent.rs`, where the decision sits immediately **before** the `auto_approve` check in the tool loop: a `Deny` refuses without consulting `auto_approve` at all — which is what makes the rules usable for `run_headless`, whose agent auto-approves unconditionally — an `Allow` skips the prompt, and `Ask`, which an unconfigured section always returns, falls through to the pre-existing approval flow untouched. Rules are compiled once at startup and shared with `ReplState` so `/permissions` shows exactly what the agent is deciding with; anything the configuration said that could not be honoured (an unknown tool name, a pattern on a tool with no gated argument, an unparseable rule, a `default_mode` agent-cli has no equivalent for) becomes a warning printed at startup rather than a silent no-op.

- **Layered configuration**: `ConfigSource` carries an ordered chain of files rather than one path — `~/.config/agent-cli/config.toml` as the base and `./.agent-cli/config.toml` as the overlay, or the files given by a repeatable `--config`, which replaces the chain entirely. `config::merge` combines them as `toml::Value` *before* `Config` is deserialized, so every section is layered without its own merge code: tables merge recursively, scalars and arrays are replaced, and `permissions.deny` / `permissions.allow` union — the one exception, because otherwise a project file could delete a machine-wide deny rule by redefining the list. `DEFAULT_CONFIG` is generated only when no layer exists at all, so a project file never conjures a user-level one. `spawn_detached` passes **every** layer to the child, one `--config` each: handing it only the top layer would drop the base and with it any machine-wide `deny`, leaving a detached agent running under weaker rules than its parent with nothing on screen to say so.

- **Mouse selection of the log**: because reporting is on for the whole session, a drag never reaches the terminal — so `select.rs` provides the one agent-cli makes itself. It is pure: a `Selection` is two `Point`s in viewport coordinates, `row_span` says which columns of a given row the range covers, `highlight` re-emits a styled row with that span in reverse video while preserving the row's own SGR state, and `selected_text` reduces the covered rows to plain text, dropping every SGR sequence and **undoing the wrap** (a row that continues the line above it is concatenated rather than newline-joined), so a line pastes as the line it was. `app.rs` owns only the gesture and the screen: `Down(Left)` on the body begins a drag — entering the alternate screen even at offset 0, since the live screen cannot be repainted with a highlight on it — `Drag` extends it and sets the same dirty flag a wheel burst uses (one repaint per drain), `Up` extracts the text and hands it to `clip.rs`. The selection lives on `ScrollOverlay` beside the view offset, which is what lets `draw_scroll_overlay` paint it without a signature change, and it is dropped when the view moves, since a range made at one offset does not describe the rows at another. `body_slice_at` produces the padded slice both the drawing and the copy read, so the highlight and the clipboard can never disagree about which row is which. `clip.rs` delivers: OSC 52 by default (a ~20-line base64, wrapped in tmux's passthrough when `TMUX` is set) so the text lands on the clipboard of whoever is looking at the session — over SSH included — or piped to `[ui] copy_command` when one is configured. OSC 52 is fire-and-forget, so an over-sized selection is refused with its limit named rather than truncated into a silent half-copy, and the reported line says which route was used.
...
§8.1 – §8.3 below cover separate subsystems — the child-process backend, the
self-update path and the MCP client — rather than further context-efficiency
features. They keep their numbers because released CHANGELOG entries cite them.
...
falling back to `tags`), a `cargo install --git <repo> --branch|--tag <ref>
agent-cli --root <prefix>`, and a `--version` verify of the result. The target
ref comes from the pure `target_ref`, which defaults to `DEFAULT_REF` (`main`)
when `--ref` is absent, so a bare `agent-cli update` equals `--ref main`; the
release lookup is then only the banner, and `--check` is the one path that
requires it.
...
mirrors that: it rebuilds from the target ref into the running binary's prefix
