...
- Select and copy with the mouse — **drag over the log** and the range is highlighted as you go; releasing copies it to the clipboard as plain text, with the colours stripped and the terminal's wrapping undone, so a wrapped line pastes as one line. It works in the live view and while scrolled back. The text travels by OSC 52, so it reaches the clipboard of the machine you are looking at even over SSH; `[ui] copy_command` pipes it to `wl-copy` / `xclip` instead, and `[ui] mouse_select` turns it off.
...
| Mouse drag | Select a range of the log; releasing copies it to the clipboard (`[ui] mouse_select`). `Shift`-drag selects with the terminal instead |
