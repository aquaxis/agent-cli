...
- **Live `thinking` view**: while the indicator is on (and `[ui] show_thinking` is not `hidden`), `AgentEvent::Thinking` is no longer printed inline — `display_event` skips it and the display task feeds it to the indicator, which buffers the turn's reasoning and renders its tail beneath the spinner. `editor::wrap_display` folds the text to `terminal_width - 3` columns, the last `THINKING_ROWS` (10) rows are shown with a `… +N more` marker, and the whole block is bounded by `terminal_height - 5` so expanding can never push the conversation off the screen. Because the block is now several rows tall, `clear` moves up `painted_rows - 1` before erasing. Clicking it toggles the tail view against the full one: `RawModeGuard` leaves mouse reporting alone, the indicator enables it in `start()` and releases it in `finish()` / `abandon_run()` (with a `DisableMouseCapture` in the guard's `Drop` as a safety net), so text selection only needs Shift while a turn is actually running. The input loop reads `CtEvent::Mouse`, resolves the block's rows with one `cursor::position()` query — the display task is the last writer during a turn, so the cursor stands on the block's final row — and tests the click with the pure `click_hits_indicator`.
