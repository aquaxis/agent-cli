...
- FR-03-1-2 の Ollama バックエンドが `message.thinking` フィールドを `ProviderEvent::Thinking` として emit すること。`Capabilities::thinking` がユーザーから観測可能な範囲で正しく反映されること。`doc/providers/ollama.md` に `glm-5.1:cloud` 等の thinking 対応モデルでの動作が記載されていること。
